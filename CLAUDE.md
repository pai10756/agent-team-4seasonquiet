...
| 規則 | 說明 |
|------|------|
| 圖卡一（hook） | **年輕女生 + 3D 小靜同框**。女生外觀每集隨機，來源：農民曆工廠 `build_random_appearance()` 隨機組合臉型/眼型/眉型/鼻型/唇型/髮型/妝容/服裝（韓系清新風格）。小靜帶 `3d_reference_clean.jpg` 壓縮參考生成，佔畫面 ~15%，女生佔 ~40%。真實攝影場景背景 |
